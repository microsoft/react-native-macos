...
struct IJSValueReader : Windows::Foundation::IInspectable {
  ...
  JSValueType ValueType() const noexcept {
    return get_itf()->ValueType();
  }
  bool GetNextObjectProperty(hstring &propertyName) const noexcept {
    return get_itf()->GetNextObjectProperty(propertyName);
  }
  bool GetNextArrayItem() const noexcept {
    return get_itf()->GetNextArrayItem();
  }
  hstring GetString() const noexcept {
    return get_itf()->GetString();
  }
  bool GetBoolean() const noexcept {
    return get_itf()->GetBoolean();
  }
  int64_t GetInt64() const noexcept {
    return get_itf()->GetInt64();
  }
  double GetDouble() const noexcept {
    return get_itf()->GetDouble();
  }

  bool GetNextObjectProperty(
      const std::wstring_view &propertyName) const noexcept {
...
struct IJSValueWriter : Windows::Foundation::IInspectable {
  ...
  void WriteBoolean(bool value) const noexcept {
    get_itf()->WriteBoolean(value);
  }
  void WriteInt64(int64_t value) const noexcept {
    get_itf()->WriteInt64(value);
  }
  void WriteDouble(double value) const noexcept {
    get_itf()->WriteDouble(value);
  }
  void WriteString(const winrt::hstring &value) const noexcept {
    get_itf()->WriteString(value);
  }
  void WriteObjectBegin() const noexcept {
    get_itf()->WriteObjectBegin();
  }
  void WritePropertyName(const winrt::hstring &name) const noexcept {
    get_itf()->WritePropertyName(name);
  }
  void WriteObjectEnd() const noexcept {
    get_itf()->WriteObjectEnd();
  }
  void WriteArrayBegin() const noexcept {
    get_itf()->WriteArrayBegin();
  }
  void WriteArrayEnd() const noexcept {
    get_itf()->WriteArrayEnd();
  }
...
using ReactModuleProvider = std::function<Windows::Foundation::IInspectable(
    const IReactModuleBuilder &)>;

struct IReactPackageBuilder : Windows::Foundation::IInspectable {
  struct Itf : Windows::Foundation::IInspectable::Itf {
    virtual void AddModule(
        const hstring &moduleName,
        const ReactModuleProvider &moduleProvider) noexcept = 0;
  };

  void AddModule(
      const hstring &moduleName,
      const ReactModuleProvider &moduleProvider) const noexcept {
    get_itf()->AddModule(moduleName, moduleProvider);
  }
...
struct IReactDispatcher : Windows::Foundation::IInspectable {
  ...
  bool HasThreadAccess() const noexcept {
    return get_itf()->HasThreadAccess();
  }
  void Post(const ReactDispatcherCallback &callback) const noexcept {
    return get_itf()->Post(callback);
  }
...
using ReactCreatePropertyValue =
    std::function<Windows::Foundation::IInspectable()>;
...
struct IReactPropertyName : Windows::Foundation::IInspectable {
  ...
  hstring LocalName() const noexcept {
    return get_itf()->LocalName();
  }
  IReactPropertyNamespace Namespace() const noexcept {
    return get_itf()->Namespace();
  }
...
struct IReactPropertyBag : Windows::Foundation::IInspectable {
  struct Itf : Windows::Foundation::IInspectable::Itf {
    ...
    virtual IInspectable GetOrCreate(
        IReactPropertyName name,
        const ReactCreatePropertyValue &createValue) noexcept = 0;
    virtual IInspectable Set(
        IReactPropertyName name,
        IInspectable value) noexcept = 0;
  };

  IInspectable Get(IReactPropertyName name) const noexcept {
    return get_itf()->Get(name);
  }
  IInspectable GetOrCreate(
      IReactPropertyName name,
      const ReactCreatePropertyValue &createValue) const noexcept {
    return get_itf()->GetOrCreate(name, createValue);
  }
  IInspectable Set(IReactPropertyName name, IInspectable value) const noexcept {
    return get_itf()->Set(name, value);
  }
...
struct ReactPropertyBagHelper {
  ...
  static IReactPropertyNamespace GetNamespace(
      const hstring &namespaceName) noexcept;
  static IReactPropertyName GetName(
      IReactPropertyNamespace ns,
      const hstring &localName) noexcept;
...
struct IReactNotificationSubscription : Windows::Foundation::IInspectable {
  ...
  IReactPropertyName NotificationName() const noexcept {
    return get_itf()->NotificationName();
  }
  IReactDispatcher Dispatcher() const noexcept {
    return get_itf()->Dispatcher();
  }
  bool IsSubscribed() const noexcept {
    return get_itf()->IsSubscribed();
  }
  void Unsubscribe() const noexcept {
    return get_itf()->Unsubscribe();
  }
...
using ReactNotificationHandler = std::function<
    void(Windows::Foundation::IInspectable, IReactNotificationArgs)>;

struct IReactNotificationService : Windows::Foundation::IInspectable {
  struct Itf : Windows::Foundation::IInspectable::Itf {
    virtual IReactNotificationSubscription Subscribe(
        IReactPropertyName notificationName,
        IReactDispatcher dispatcher,
        const ReactNotificationHandler &handler) noexcept = 0;
    virtual void SendNotification(
        IReactPropertyName notificationName,
        IInspectable sender,
        IInspectable data) noexcept = 0;
  };

  IReactNotificationSubscription Subscribe(
      IReactPropertyName notificationName,
      IReactDispatcher dispatcher,
      const ReactNotificationHandler &handler) const noexcept {
...
struct IReactContext : Windows::Foundation::IInspectable {
  struct Itf : Windows::Foundation::IInspectable::Itf {
    ...
    virtual void CallJSFunction(
        const hstring &moduleName,
        const hstring &methodName,
        const JSValueArgWriter &paramsArgWriter) noexcept = 0;
    virtual void EmitJSEvent(
        const hstring &eventEmitterName,
        const hstring &eventName,
        const JSValueArgWriter &paramsArgWriter) noexcept = 0;
  };

  IReactPropertyBag Properties() const noexcept {
    return get_itf()->Properties();
  }
  IReactNotificationService Notifications() const noexcept {
    return get_itf()->Notifications();
  }
  IReactDispatcher UIDispatcher() const noexcept {
    return get_itf()->UIDispatcher();
  }
  IReactDispatcher JSDispatcher() const noexcept {
    return get_itf()->JSDispatcher();
  }
  void CallJSFunction(
      const hstring &moduleName,
      const hstring &methodName,
      const JSValueArgWriter &paramsArgWriter) const noexcept {
    return get_itf()->CallJSFunction(moduleName, methodName, paramsArgWriter);
  }
  void EmitJSEvent(
      const hstring &eventEmitterName,
      const hstring &eventName,
      const JSValueArgWriter &paramsArgWriter) const noexcept {
    return get_itf()->EmitJSEvent(eventEmitterName, eventName, paramsArgWriter);
  }
...
using MethodDelegate = std::function<void(
    IJSValueReader,
    IJSValueWriter,
...
struct IReactModuleBuilder : Windows::Foundation::IInspectable {
  struct Itf : Windows::Foundation::IInspectable::Itf {
    virtual void AddInitializer(
        const InitializerDelegate &initializer) noexcept = 0;
    virtual void AddConstantProvider(
        const ConstantProviderDelegate &constantProvider) noexcept = 0;
    virtual void AddMethod(
        const hstring &name,
        MethodReturnType returnType,
        const MethodDelegate &method) noexcept = 0;
    virtual void AddSyncMethod(
        const hstring &name,
        const SyncMethodDelegate &method) noexcept = 0;
  };

  void AddInitializer(InitializerDelegate initializer) const noexcept {
    return get_itf()->AddInitializer(initializer);
  }
  void AddConstantProvider(
      ConstantProviderDelegate constantProvider) const noexcept {
    return get_itf()->AddConstantProvider(constantProvider);
  }
  void AddMethod(
      const hstring &name,
      MethodReturnType returnType,
      MethodDelegate method) const noexcept {
    return get_itf()->AddMethod(name, returnType, method);
  }
  void AddSyncMethod(const hstring &name, SyncMethodDelegate method)
      const noexcept {
    return get_itf()->AddSyncMethod(name, method);
  }
  ...
  void AddSyncMethod(const std::wstring_view &name, SyncMethodDelegate method)
      const noexcept {
    auto str = std::wstring(name.cbegin(), name.cend());
...
extern IReactContext CreateMacOSReactContext(
    const std::shared_ptr<facebook::react::CallInvoker> &jsInvoker) noexcept;
