...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    ...
    _view = [[RCTPlatformView alloc] init]; // [macOS]
...
- (void)onChange:(RCTPlatformView *)sender // [macOS]
{
  // No-op
...
{
#if !TARGET_OS_OSX // [macOS
  _view.backgroundColor = RCTUIColorFromHexString(std::string(color.UTF8String));
#else // [macOS
  _view.layer.backgroundColor = RCTUIColorFromHexString(std::string(color.UTF8String)).CGColor;
#endif // macOS]
}
