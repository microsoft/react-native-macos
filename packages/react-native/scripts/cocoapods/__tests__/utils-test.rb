...
class UtilsTests < Test::Unit::TestCase
    ...
    # ======================================= #
    # Test - Set Supported Platforms          #
    # ======================================= #
    def test_setSupportedPlatformsForFrameworks_whenMultiPlatformFrameworks_setsSupportedPlatformsFromSDKROOT
        # Arrange
        ENV['USE_FRAMEWORKS'] = 'dynamic'
        $RN_PLATFORMS = ['iOS', 'macOS', 'visionOS']
        ios_target = TargetMock.new("hermes-engine-iOS", [
            prepare_config("Debug", {"SDKROOT" => "iphoneos"}),
            prepare_config("Release", {"SDKROOT" => "iphoneos"}),
        ])
        macos_target = TargetMock.new("hermes-engine-macOS", [
            prepare_config("Debug", {"SDKROOT" => "macosx"}),
        ])
        visionos_target = TargetMock.new("hermes-engine-visionOS", [
            prepare_config("Debug", {"SDKROOT" => "xros"}),
        ])
        unknown_target = TargetMock.new("SomeHostTool", [
            prepare_config("Debug", {"SDKROOT" => "someunknownsdk"}),
        ])
        pods_projects_mock = PodsProjectMock.new([], {}, native_targets: [ios_target, macos_target, visionos_target, unknown_target])
        installer = InstallerMock.new(pods_projects_mock, [])

        # Act
        ReactNativePodsUtils.set_supported_platforms_for_frameworks(installer)

        # Assert
        ios_target.build_configurations.each { |config| assert_equal("iphoneos iphonesimulator", config.build_settings["SUPPORTED_PLATFORMS"]) }
        macos_target.build_configurations.each { |config| assert_equal("macosx", config.build_settings["SUPPORTED_PLATFORMS"]) }
        visionos_target.build_configurations.each { |config| assert_equal("xros xrsimulator", config.build_settings["SUPPORTED_PLATFORMS"]) }
        unknown_target.build_configurations.each { |config| assert_nil(config.build_settings["SUPPORTED_PLATFORMS"]) }
        assert_equal(1, pods_projects_mock.save_invocation_count)
    end

    def test_setSupportedPlatformsForFrameworks_whenNotUsingFrameworks_doesNothing
        # Arrange
        ENV['USE_FRAMEWORKS'] = nil
        $RN_PLATFORMS = ['iOS', 'macOS', 'visionOS']
        macos_target = TargetMock.new("hermes-engine-macOS", [prepare_config("Debug", {"SDKROOT" => "macosx"})])
        pods_projects_mock = PodsProjectMock.new([], {}, native_targets: [macos_target])
        installer = InstallerMock.new(pods_projects_mock, [])
        ...
        # Assert
        macos_target.build_configurations.each { |config| assert_nil(config.build_settings["SUPPORTED_PLATFORMS"]) }
        assert_equal(0, pods_projects_mock.save_invocation_count)
    end

    def test_setSupportedPlatformsForFrameworks_whenSinglePlatform_doesNothing
        # Arrange
        ENV['USE_FRAMEWORKS'] = 'dynamic'
        $RN_PLATFORMS = ['iOS']
        ios_target = TargetMock.new("hermes-engine-iOS", [prepare_config("Debug", {"SDKROOT" => "iphoneos"})])
        pods_projects_mock = PodsProjectMock.new([], {}, native_targets: [ios_target])
        installer = InstallerMock.new(pods_projects_mock, [])
        ...
        # Assert
        ios_target.build_configurations.each { |config| assert_nil(config.build_settings["SUPPORTED_PLATFORMS"]) }
        assert_equal(0, pods_projects_mock.save_invocation_count)
    end
