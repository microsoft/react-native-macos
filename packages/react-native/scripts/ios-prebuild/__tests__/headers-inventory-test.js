...
const {computeInventory, scanHeader} = require('../headers-inventory');
const path = require('path');

test('fork inventory retains stable dispatcher paths and macOS headers without collisions', () => {
  const inventory = computeInventory(path.resolve(__dirname, '../../..'));
  expect(inventory.collisions).toEqual([]);
  const byPath = new Map(inventory.headers.map(h => [h.naturalPath, h]));
  const base = 'react/renderer/components/view/';
  for (const name of [
    'HostPlatformTouch',
    ...
    expect(byPath.has(`${base}platform/cxx/${base}${name}.h`)).toBe(true);
  }
  expect(byPath.has('React/RCTUIKit.h')).toBe(true);
  expect(inventory.headers.flatMap(h => h.includes.unresolved)).not.toEqual(
    expect.arrayContaining(['AppKit/AppKit.h']),
  );
});
