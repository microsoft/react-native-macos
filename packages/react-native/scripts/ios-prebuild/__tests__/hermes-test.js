...
test.each([
  ['0', metadata.HERMES_VERSION_NAME],
  ['1', metadata.HERMES_V1_VERSION_NAME],
  ['', metadata.HERMES_V1_VERSION_NAME],
  ['true', metadata.HERMES_V1_VERSION_NAME],
])('selects stable metadata with flag %s', async (flag, version) => {
  properties = checkedInProperties;
  process.env.RCT_HERMES_V1_ENABLED = flag;
  await prepareHermesArtifactsAsync('0.84.1', 'Debug');
...
test('the 0.84 caller builds selected metadata 1000.0.0 from source', async () => {
  properties = 'HERMES_VERSION_NAME=0.15.1\nHERMES_V1_VERSION_NAME=1000.0.0';
