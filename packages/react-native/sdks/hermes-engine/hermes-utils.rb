...
def podspec_source_build_from_github_tag(react_native_path)
    source_ref = File.read(hermestag_file(react_native_path)).strip
    # [macOS] CocoaPods must fetch full commit SHAs as commits, not tags.
    ref_type = source_ref.match?(/\A[0-9a-fA-F]{40}\z/) ? :commit : :tag

    hermes_log("Using #{ref_type} defined in sdks/.hermesv1version: #{source_ref}")
    return {:git => HERMES_GITHUB_URL, ref_type => source_ref}
end
